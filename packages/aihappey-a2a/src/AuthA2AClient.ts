...
export class AuthA2AClient {
     ...
     */
    //constructor(agentBaseUrl, getToken, agentCard: AgentCard | undefined = undefined) {
    constructor(agentBaseUrl, getToken) {
        this.agentBaseUrl = agentBaseUrl.replace(/\/$/, ""); // Remove trailing slash if any
        //    this.agentCardPromise = this._fetchAndCacheAgentCard();
        /*   if (agentCard) {
               // Immediately resolve/cach the agentCard and endpoint
    ...
    async _fetchAndCacheAgentCard() {
        ...
        try {
            var token = this.getToken ? await this.getToken() : undefined;
    ...
    async getAgentCard(agentBaseUrl) {
        if (agentBaseUrl) {
            ...
            const agentCardUrl = `${specificAgentBaseUrl}/.well-known/agent-card.json`;
    ...
    async _postRpcRequest(method, params) {
        ...
        if (!httpResponse.ok) {
            ...
            try {
                ...
                if ((e as Error).message.startsWith('RPC error for') || (e as Error).message.startsWith('HTTP error for'))
                    throw e;
                throw new Error(`HTTP error for ${method}! Status: ${httpResponse.status} ${httpResponse.statusText}. Response: ${errorBodyText}`);
            }
        }
    ...
    async *sendMessageStream(params) {
        ...
        const endpoint = await this._getServiceEndpoint();
        const clientRequestId = String(this.requestIdCounter++); // Use a unique ID for this stream request
        const rpcRequest = {
            jsonrpc: "2.0",
        ...
        if (!response.ok) {
            ...
            try {
                ...
                if ((e as Error).message.startsWith('HTTP error establishing stream'))
                    throw e;
    ...
    async *resubscribeTask(params) {
        ...
        const endpoint = await this._getServiceEndpoint();
        const clientRequestId = String(this.requestIdCounter++);
        const rpcRequest = {
            jsonrpc: "2.0",
    ...
    async *_parseA2ASseStream(response, originalRequestId) {
        ...
        try {
            ...
            console.error("Error reading or parsing SSE stream:", (error as Error).message);
            throw error;
        }
        finally {
    ...
    _processSseEventData(jsonData, originalRequestId) {
        ...
        try {
            ...
            if ((e as Error).message.startsWith("SSE event contained an error") || (e as Error).message.startsWith("SSE event JSON-RPC response is missing 'result' field")) {
            ...
            throw new Error(`Failed to parse SSE event data: "${jsonData.substring(0, 100)}...". Original error: ${(e as Error).message}`);
        }
    }
