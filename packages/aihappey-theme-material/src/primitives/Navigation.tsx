...
const NavRow = ({ item, activeKey, onSelect, depth = 0 }: { item: NavigationItem; activeKey?: string; onSelect?: (key: string) => void; depth?: number }) => {
  ...
  return <><ListItemButton selected={selected} disabled={item.disabled} sx={{ pl: 2 + depth * 2 }} onClick={(event) => { item.onClick?.(event as any); if (hasChildren) setOpen((value: boolean) => !value); else onSelect?.(itemValue); }}><ListItemIcon sx={{ minWidth: 32 }}>{renderIcon(item.icon)}</ListItemIcon><ListItemText primary={item.label} />{hasChildren ? renderIcon(open ? "chevronUp" : "chevronDown") : null}</ListItemButton>{hasChildren ? <Collapse in={open} timeout="auto" unmountOnExit><List disablePadding>{item.children?.map((child: NavigationItem, index: number) => <NavRow key={`${child.key ?? child.eventKey ?? index}:${index}`} item={child} activeKey={activeKey} onSelect={onSelect} depth={depth + 1} />)}</List></Collapse> : null}</>;
};
