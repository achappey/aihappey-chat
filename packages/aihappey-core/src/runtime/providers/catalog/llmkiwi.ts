import type { Provider } from "aihappey-types";

export const llmkiwi: Provider = {
  name: "LLMkiwi",
  description: "Build with LLM.kiwi using AI tools, a transparent model catalog, and an OpenAI-compatible API for production chat and automation workflows.",
  icons: [{
    src: "https://llm.kiwi/logo.svg"
  }],
  urls: {
    homepage: "https://llm.kiwi",
    docs: "https://llm.kiwi/docs",
    console: "https://llm.kiwi/dashboard",
    pricing: "https://llm.kiwi/pricing",
    privacyPolicy: "https://llm.kiwi/privacy-policy",
    termsOfService: "https://llm.kiwi/terms-of-service"
  },
  providerCountry: "HR",
  inferenceRegions: ["World"]

};

