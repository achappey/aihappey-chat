...
.dark .aih-shadcn-theme {
  ...
  --aih-shadcn-card: hsl(222.2 84% 4.9%);
...
.aih-shadcn-theme,
.aih-shadcn-theme * { box-sizing: border-box; }
...
.aih-shadcn-card-header { display: flex; align-items: flex-start; justify-content: space-between; gap: .75rem; padding: 1rem 1rem .5rem; }
...
.aih-shadcn-popover { z-index: 50; min-width: 12rem; border: 1px solid var(--aih-shadcn-border); border-radius: var(--aih-shadcn-radius); background: var(--aih-shadcn-popover); color: var(--aih-shadcn-popover-foreground); box-shadow: 0 10px 30px rgb(0 0 0 / .18); padding: .25rem; }
.aih-shadcn-menu-item { display: flex; align-items: center; gap: .5rem; width: 100%; border: 0; border-radius: calc(var(--aih-shadcn-radius) - 4px); background: transparent; color: inherit; padding: .5rem .625rem; font-size: .875rem; text-align: left; cursor: pointer; }
.aih-shadcn-menu-item:hover,
.aih-shadcn-menu-item[data-highlighted] { background: var(--aih-shadcn-accent); }
.aih-shadcn-menu-item-danger { color: var(--aih-shadcn-destructive); }

.aih-shadcn-dialog-overlay { position: fixed; inset: 0; z-index: 50; background: rgb(0 0 0 / .55); }
.aih-shadcn-dialog-content { position: fixed; left: 50%; top: 50%; z-index: 51; width: min(calc(100vw - 2rem), 36rem); transform: translate(-50%, -50%); border: 1px solid var(--aih-shadcn-border); border-radius: var(--aih-shadcn-radius); background: var(--aih-shadcn-background); color: var(--aih-shadcn-foreground); box-shadow: 0 20px 60px rgb(0 0 0 / .25); padding: 1rem; }
.aih-shadcn-drawer-content { position: fixed; inset-block: 0; right: 0; z-index: 51; width: min(28rem, 90vw); border-left: 1px solid var(--aih-shadcn-border); background: var(--aih-shadcn-background); color: var(--aih-shadcn-foreground); box-shadow: -10px 0 30px rgb(0 0 0 / .18); padding: 1rem; overflow: auto; }
...
.aih-shadcn-chat-message-user { align-self: flex-end; background: var(--aih-shadcn-primary); color: var(--aih-shadcn-primary-foreground); }
.aih-shadcn-chat-message-assistant { align-self: flex-start; }
...
.aih-shadcn-toast-viewport { position: fixed; right: 1rem; bottom: 1rem; z-index: 100; display: flex; max-width: min(24rem, calc(100vw - 2rem)); flex-direction: column; gap: .5rem; margin: 0; padding: 0; list-style: none; }
.aih-shadcn-toast-root { border: 1px solid var(--aih-shadcn-border); border-radius: var(--aih-shadcn-radius); background: var(--aih-shadcn-background); color: var(--aih-shadcn-foreground); padding: .75rem; box-shadow: 0 10px 30px rgb(0 0 0 / .15); }
`;
