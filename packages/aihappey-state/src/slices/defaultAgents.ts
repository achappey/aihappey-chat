...
export const conversationNameAgent = {
...
export const explainToolcallAgent = {
...
export const ensureDefaultAgents = (agents: Agent[] = []): Agent[] => {
  const existingNames = new Set((agents ?? []).map((agent) => agent?.name).filter(Boolean));
  const missingDefaults = defaultAgents
