...
const NavigationRow = ({ item, activeKey, onSelect, onRename, onDelete, onExport, onTogglePin, translations }: any) => {
  ...
  return <div><button type="button" disabled={item.disabled} aria-current={navItemMatchesActive(item, activeKey) ? "page" : undefined} onClick={() => item.onClick ? item.onClick() : onSelect?.(itemValue)}>{icon(item.icon)}{item.icon ? " " : null}{item.label}{item.new ? ` (${translations?.new ?? "new"})` : ""}</button>{item.conversationItem ? <details><summary>{item.pinned ? "★" : "…"}</summary>{onRename ? <button type="button" onClick={() => { setEditValue(String(item.label ?? "")); setEditing(true); }}>{translations?.rename ?? "Rename"}</button> : null}{onExport ? <button type="button" onClick={() => void onExport(item.key)}>{translations?.export ?? "Export"}</button> : null}{onTogglePin ? <button type="button" onClick={() => void onTogglePin(item.key)}>{item.pinned ? (translations?.unpin ?? "Unpin") : (translations?.pin ?? "Pin")}</button> : null}{onDelete ? <button type="button" onClick={() => void onDelete(item.key)}>{translations?.delete ?? "Delete"}</button> : null}</details> : null}</div>;
};
