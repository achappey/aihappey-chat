...
export const Chat = ({ messages = [], renderMessage, renderReactions, locale, aiGeneratedLabel, aiGeneratedWarning, disableProviderLogo }: any) => <VStack gap="3" align="stretch">{messages.map((msg: ChatMessage) => { const isUser = msg.role === "user"; const isAssistant = msg.role === "assistant"; const streaming = msg.content?.some((part: any) => part.type === "text" && part.state === "streaming"); const providerLogo = !disableProviderLogo && isAssistant && msg.providerIcon?.src ? msg.providerIcon : undefined; const messageIcon = !isUser && msg.messageIcon ? renderIcon(msg.messageIcon, 14) : undefined; return <Box key={msg.id} borderWidth="1px" borderRadius="lg" padding="3" maxWidth="90%" alignSelf={isUser ? "flex-end" : "flex-start"}><HStack justify="space-between" gap="2" marginBottom="1"><HStack gap="2">{providerLogo ? <AvatarGroup.Avatar size={24} shape="square" image={{ src: providerLogo.src, alt: providerLogo.alt ?? msg.providerName ?? msg.providerKey }} name={msg.providerName ?? msg.providerKey} /> : messageIcon ? <AvatarGroup.Avatar size={24} icon={messageIcon} /> : null}<TextBase fontWeight="semibold" fontSize="sm">{msg.author ?? (isUser ? "You" : "Assistant")}</TextBase>{isAssistant && aiGeneratedWarning ? <BadgeBase title={aiGeneratedWarning} variant="outline" size="xs">{aiGeneratedLabel}</BadgeBase> : null}</HStack><TextBase fontSize="xs" color="fg.muted">{msg.createdAt ? format(msg.createdAt, locale) : ""}</TextBase></HStack>{renderMessage(msg)}{streaming || renderReactions ? <Box marginTop="2">{streaming ? <ProgressBar value={100} /> : renderReactions?.(msg)}</Box> : null}</Box>; })}</VStack>;
...
const renderNavItem = (item: NavigationItem, activeKey?: string, onSelect?: (key: string) => void, path = "nav", index = 0): React.ReactNode => {
  ...
  return <Box key={key}><ButtonBase width="100%" justifyContent="flex-start" variant={selected ? "subtle" : "ghost"} disabled={item.disabled} onClick={(event: any) => { item.onClick?.(event); if (!item.children?.length) onSelect?.(itemValue); }}>{renderIcon(item.icon)}<Box as="span" minWidth="0" flex="1" overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap" textAlign="start">{item.label}</Box>{item.badge ? <BadgeBase variant="outline" size="xs" marginLeft="auto">{item.badge}</BadgeBase> : null}</ButtonBase>{item.children?.length ? <VStack align="stretch" paddingLeft="4">{item.children.map((child: NavigationItem, childIndex: number) => renderNavItem(child, activeKey, onSelect, key, childIndex))}</VStack> : null}</Box>;
};
